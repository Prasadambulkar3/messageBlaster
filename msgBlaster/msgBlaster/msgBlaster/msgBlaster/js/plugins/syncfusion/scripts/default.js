...
$("#Res-prodnav").on("click",function (e) {
    ...
    $("#sbtooltipbox").ejDialog({ height: "84px", width: "176px", enableResize: false, showOnInit: false, showHeader: false, cssClass: "metroblue", allowKeyboardNavigation:false });
        ...
        self.loadSourceCodeTab = function (url) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $("#sourceTab").ejTab({ cssClass: "metroblue" }).show();
                    ...
                    $("<span>").attr("id", "newcodewindow").attr('title','New Window').addClass(" e-icon e-select newsrcwindow").appendTo($("#sourceTab .e-header"));
                    $("<span>").attr("id", "copyclipboard").attr('title','Copy To Clipboard').addClass(" e-icon e-select copycodeicon copycode").appendTo($("#sourceTab .e-header"));
...
function onSelectSearchItem(args) {
	...
	setTimeout(function(){
	var curr = $(".highlighttextbg.selecteddashboard");
	 if (curr.length == 1) {
